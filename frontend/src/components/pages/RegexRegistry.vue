<template>
    <div class="">
        ...
        <div class="flex flex-wrap justify-center mt-10">
            <RegistryItem v-for="item in filteredRegex" :key="item.id" :name="item.name" :created="item.created"
                :tags="item.tags" :regex="item.regex" :id="item.id" @getRegex="getRegex">
            </RegistryItem>
        </div>
    </div>
</template>
...
export default {
    ...
    setup() {
        ...
        return { setFilter, tags, search, filter, filteredRegex, getRegex }
    }
}
